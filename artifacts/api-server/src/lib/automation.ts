...
export async function sendBirthdayEmail(
  ...
  try {
    ...
    const body = await generateClaudeMessage(
      `You are writing a short birthday email on behalf of ${hCtx.hospitalName} to a patient. Tone: ${tone}. IMPORTANT: Write the entire email in ${lang}. STRICT RULES: (1) Never reference age, milestones, getting older, or how many years have passed. (2) Never reference religion, God, prayers, blessings, or any spiritual practice. (3) Never reference tribe, ethnicity, culture, or traditions. (4) Never assume gender — use gender-neutral language, never say he/she/his/her. (5) Do NOT highlight or describe any personal traits, qualities, or characteristics about the person — the hospital does not know them personally, so only celebrate the day itself. (6) Never say you are happy or glad the patient is a patient. Start with "Happy Birthday ${firstName}!" then write exactly 2 short paragraphs: the first warmly wishes them a joyful day and good health; the second is a brief warm note from the team. End with a simple sign-off from ${hCtx.hospitalName}. The entire email body must be under 90 words. Do not add contact lines or "please do not reply".`,
      `Write a short, warm birthday email for ${firstName} from ${hCtx.hospitalName}. 2 paragraphs, under 90 words total. Celebrate the day only — no personal traits.`,
      180,
    );
