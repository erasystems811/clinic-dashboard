...
// ── Continuous In-Care AI Reminders — OpenAI — Email — Morning + Evening ───────
// Sent twice daily to all patients currently In Care who have a treatment plan.
// Each message is uniquely generated by OpenAI based on the patient's treatment plan.
...
export async function sendInCareAIReminder(
  ...
  try {
    ...
    const timeContext = timeOfDay === "morning"
      ? "It is morning. This could be a medication reminder, a prompt to eat well before the day starts, a gentle encouragement for the day ahead, or a reminder of any morning care instructions from their plan."
      : "It is evening. This could be a reminder about evening medication, a clinic visit tonight if mentioned in their plan, end-of-day care steps, or a warm check-in about how they are feeling today.";

    const message = await generateOpenAIMessage(
      `You are a care team member at ${hCtx.hospitalName} sending a brief ${timeOfDay} check-in email to a patient. Tone: ${tone}. Start with "Hi ${firstName},". Write 2–3 warm, specific sentences relevant to the time of day and the patient's treatment plan. Sound like a real, caring person — not an automated system. Never mention a diagnosis. Never use clinical jargon. End with "If you have any concerns please do not hesitate to ${contact}. Please do not reply to this email directly. — ${hCtx.hospitalName} Team"`,
      `${timeContext}\n\nPatient's treatment plan: ${treatmentPlan.slice(0, 600)}\n\nWrite a short, warm ${timeOfDay} reminder email for ${firstName} based on this plan.`,
      220,
    );
    ...
    await sendEmail({
      ...
      subject: timeOfDay === "morning"
        ? `Good morning, ${firstName} — ${hCtx.hospitalName}`
        : `Good evening, ${firstName} — ${hCtx.hospitalName}`,
      html,
      text: message,
    });

    await updateAutomationLog(logId, "sent", `In-care ${timeOfDay} reminder → ${patientEmail}`);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await updateAutomationLog(logId, "failed", msg);
    Sentry.captureException(err, { extra: { ...ctx } });
