    ...
    <div className="px-5 pt-6 pb-24">
          ...
          <div className="flex items-end justify-between gap-4 mb-4">
            ...
            <div className="flex-1 relative h-2 rounded-full overflow-hidden"
              ...
              <div className="h-full rounded-full transition-all duration-700"
                style={{ width: `${pctToGoal}%`, background: WL_COLOR }} />
            </div>
            <div className="text-center">
              <p className="text-2xl font-black" style={{ color: WL_COLOR }}>{profile.goalWeightKg}</p>
              <p className="text-xs" style={{ color: WL_COLOR }}>Goal kg</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <AdherenceCard
          ...
          pct={data?.avgMealAdherence ?? null}
          emoji="🍽️"
        />
        <AdherenceCard
          label="Workout adherence"
          pct={data?.avgWorkoutAdherence ?? null}
          emoji="💪"
        />
      </div>
    ...
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ overflow: "visible" }}>
        ...
        {last14.map((e, i) => {
          const x = (i / (last14.length - 1)) * W;
          const y = H - ((e.weight - min) / range) * H;
          return <circle key={i} cx={x} cy={y} r={3} fill={WL_COLOR} />;
        })}
      </svg>
