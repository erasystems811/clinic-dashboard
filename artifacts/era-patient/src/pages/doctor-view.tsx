...
import {
  Clock, Users, Calendar, PhoneCall, ArrowRightLeft, AlertTriangle, CheckCircle2, Loader2, RefreshCw, X,
} from "lucide-react";
      ...
      <div className="bg-card border border-border rounded-xl w-full max-w-sm shadow-xl">
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-muted-foreground">
            Transfer <strong>{entry.patientName}</strong> to another doctor:
          </p>
          <select
          ...
          {others.length === 0 && (
            <p className="text-xs text-amber-500">No other available doctors.</p>
          )}
...
export default function DoctorView() {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      const r = await fetch(apiUrl(`/api/doctor/queue?doctorId=${doctorId}`), {
  ...
  const fetchAppointments = useCallback(async () => {
    if (!token || !doctorId) return;
    try {
      const r = await fetch(apiUrl(`/api/doctor/appointments?doctorId=${doctorId}`), {
        headers: { "x-hospital-token": token },
      });
    ...
    <Layout>
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="flex gap-1 p-1 bg-muted rounded-lg w-fit">
          ...
          >
            <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> My Queue</span>
          </button>
          <button
          ...
          >
            <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> My Appointments</span>
          </button>
        </div>
        ...
        {activeTab === "queue" && (
          <div className="rounded-lg border border-border bg-card">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
              ...
              <button onClick={fetchQueue} className="text-muted-foreground hover:text-foreground ml-1">
                  ...
                  <div key={entry.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                        ...
                        <span>{waitTime(entry.addedAt)}</span>
                        {entry.calledInAt && (
                          <span className="text-emerald-500 font-medium">· Called in</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <Button
                        size="sm"
                        className="gap-1.5 text-xs h-8"
                        disabled={callingIn === entry.id || !!entry.calledInAt}
