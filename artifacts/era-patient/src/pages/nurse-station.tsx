...
export default function NurseStation() {
  ...
  const executeEndTreatment = async () => {
    ...
    try {
      ...
      toast({ title: "Treatment ended", description: `${selectedPatient.firstName} ${selectedPatient.lastName} moved to Post Treatment. You can log a new plan immediately.` });
      queryClient.invalidateQueries({ queryKey: getListPatientsQueryKey() });
      queryClient.invalidateQueries({ queryKey: getListQueueQueryKey() });
      setSelectedPatient(null);
      setSearch("");
                ...
                {!!selectedPatient.treatmentPlan && (
                  <div className="rounded-lg border border-purple-500/30 bg-purple-500/5 p-3.5 space-y-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm font-medium text-purple-300">Patient has an active treatment plan</p>
                    </div>
                    {!confirmEndTreatment ? (
                      <Button
                        type="button"
                      ...
                      >
                        ...
                        {endingTreatment ? "Updating..." : "End Treatment Early"}
                      </Button>
                    ) : (
                      <div className="space-y-2">
                        <div className="flex items-start gap-2 p-2.5 rounded-md bg-amber-500/10 border border-amber-500/20">
                          <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                          <p className="text-xs text-amber-300">This moves <span className="font-semibold">{selectedPatient.firstName}</span> to Post Treatment. You can log a new plan for them right after.</p>
                        </div>
                        <div className="flex gap-2">
                          ...
                          <Button type="button" size="sm" className="flex-1 text-xs bg-purple-600 hover:bg-purple-700 text-white border-0" onClick={executeEndTreatment} disabled={endingTreatment}>
                            {endingTreatment ? "Updating..." : "Yes, end early"}
                          </Button>
                        </div>
                      </div>
                    )}
