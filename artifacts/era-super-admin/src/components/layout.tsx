  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <aside className="shrink-0 flex flex-col transition-all duration-200"
        ...
        {/* Nav items */}
        <nav className={`flex-1 overflow-y-auto overflow-x-hidden ${open ? "px-3 pt-3 pb-2 space-y-0.5" : "px-2 py-3 space-y-1"}`}>
          {NAV.map(item => {
        ...
        <div className="px-3 pb-3 space-y-0.5 border-t pt-2" style={{ borderTopColor: sbBorder }}>
          ...
          <button onClick={toggleOpen} title={open ? "Collapse" : "Expand"}
            className={`w-full flex items-center text-muted-foreground/30 hover:text-muted-foreground/50 transition-all duration-150 rounded-lg ${
              open ? "gap-3 px-3 py-1.5" : "justify-center p-2.5"
            }`}>
            {open
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top bar */}
        <header className="shrink-0 h-12 border-b flex items-center px-6 gap-2"
