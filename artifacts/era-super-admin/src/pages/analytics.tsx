            ...
            {health.checks.map(c => {
              ...
              return (
                <div
                  ...
                  className={`rounded border p-2.5 cursor-default select-none transition ${
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <Icon className={`w-3.5 h-3.5 ${isDown ? "text-red-400" : isWarn ? "text-amber-400" : "text-emerald-400"}`} />
                    <div className="flex items-center gap-1">
                      {c.flagged && <ShieldAlert className="w-3 h-3 text-red-400" title="Manually flagged" />}
                      <span className={`w-1.5 h-1.5 rounded-full ${isDown ? "bg-red-400" : isWarn ? "bg-amber-400 animate-pulse" : "bg-emerald-400"}`} />
                    </div>
                  </div>
                  <p className="text-xs font-bold text-foreground leading-tight">{c.name}</p>
                  <p className={`text-[10px] mt-0.5 font-medium leading-snug truncate ${isDown ? "text-red-400" : isWarn ? "text-amber-400" : "text-muted-foreground"}`}>
                    {c.balance ?? c.detail}
                  </p>
                </div>
              );
