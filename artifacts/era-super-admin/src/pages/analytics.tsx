            ...
            {health.checks.map(c => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-2.5 cursor-default select-none transition ${
                    isDown ? "bg-red-500/5" : isWarn ? "bg-amber-500/5" : ""
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 shrink-0 ${isDown ? "text-red-400" : isWarn ? "text-amber-400" : "text-emerald-400"}`} />
                  <p className="text-xs font-semibold text-foreground truncate flex-1">{c.name}</p>
                  <p className={`text-[11px] font-medium truncate max-w-[180px] ${isDown ? "text-red-400" : isWarn ? "text-amber-400" : "text-muted-foreground"}`}>
                    {c.balance ?? c.detail}
                  </p>
                  <div className="flex items-center gap-1.5 shrink-0">
                    {c.flagged && <ShieldAlert className="w-3 h-3 text-red-400" title="Manually flagged" />}
                    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isDown ? "bg-red-400" : isWarn ? "bg-amber-400 animate-pulse" : "bg-emerald-400"}`} />
                  </div>
                </div>
              );
            })}
