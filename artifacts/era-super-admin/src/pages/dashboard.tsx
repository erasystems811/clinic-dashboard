    ...
    <Layout>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Hospitals</h1>
          <p className="text-sm text-muted-foreground mt-1.5">
            All hospital accounts on the ERA platform
          </p>
        </div>
        <div className="flex items-center gap-2 mt-1">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded bg-primary text-primary-foreground text-sm font-bold hover:bg-primary/90 transition"
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/50 transition font-medium"
          />
        </div>
        <button
          onClick={() => setShowSuspended(s => !s)}
          className={`flex items-center gap-1.5 px-3 py-2 rounded border text-xs font-bold transition whitespace-nowrap uppercase tracking-wide ${
            showSuspended
                  ...
                  >
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded bg-primary/10 ring-1 ring-primary/15 flex items-center justify-center shrink-0">
      ...
      {!loading && !error && hospitals.length > 0 && (
        <div className="flex items-center gap-4 mt-3 px-1">
          {[
            { label: "Total", value: hospitals.length, color: "text-muted-foreground" },
            { label: "Active", value: hospitals.filter(h => h.active && h.subscriptionStatus === "active").length, color: "text-emerald-400" },
            { label: "Trial", value: hospitals.filter(h => h.active && h.subscriptionStatus === "trial").length, color: "text-amber-400" },
            { label: "Suspended", value: hospitals.filter(h => !h.active || h.subscriptionStatus === "inactive").length, color: "text-red-400" },
          ].map(s => (
            <span key={s.label} className="text-[11px] font-bold text-muted-foreground/50 uppercase tracking-widest">
