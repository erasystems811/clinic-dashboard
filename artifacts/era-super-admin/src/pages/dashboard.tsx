...
export default function Hospitals() {
  ...
  return (
    <Layout>
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2 mt-1">
          <button onClick={fetchHospitals}
            className="p-1.5 border border-[hsl(220_14%_24%)] text-muted-foreground/50 hover:text-muted-foreground hover:border-[hsl(220_14%_30%)] transition"
            title="Refresh">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="relative flex-1 max-w-72">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full pl-9 pr-3 py-2 bg-[hsl(220_14%_20%)] border border-[hsl(220_14%_24%)] text-[11px] text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:border-primary/40 focus:ring-1 focus:ring-primary/20 transition tracking-wide" />
        </div>
        <button onClick={() => setShowSuspended(s => !s)}
          className={`flex items-center gap-1.5 px-3 py-2 border text-[10px] font-bold uppercase tracking-[0.15em] transition ${
            showSuspended
              ...
              : "border-[hsl(220_14%_24%)] bg-transparent text-muted-foreground/50 hover:text-muted-foreground hover:border-[hsl(220_14%_30%)]"
          }`}>
      ...
      {/* Table container */}
      <div className="border border-[hsl(220_14%_30%)] overflow-hidden">

        {/* Table header */}
        <div className="border-b border-[hsl(220_14%_30%)] bg-[hsl(220_14%_14%)] grid grid-cols-[1fr_140px_120px_130px_110px_32px] gap-0">
        ...
        {loading ? (
          <div className="flex items-center justify-center gap-2.5 py-16 bg-[hsl(220_14%_18%)]">
            <Loader2 className="w-4 h-4 animate-spin text-muted-foreground/40" />
            <span className="text-[11px] text-muted-foreground/40 uppercase tracking-[0.2em]">Loading registry…</span>
          </div>
        ) : error ? (
          <div className="flex items-center justify-center gap-2 py-16 bg-[hsl(220_14%_18%)]">
            <AlertCircle className="w-4 h-4 text-destructive/70" />
            <span className="text-[11px] text-destructive/70">{error}</span>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 bg-[hsl(220_14%_18%)]">
          ...
          <div className="divide-y divide-[hsl(220_14%_18%)]">
            {filtered.map(h => {
              ...
              return (
                <div key={h.id}
                  className="grid grid-cols-[1fr_140px_120px_130px_110px_32px] items-center cursor-pointer group hover:bg-[hsl(220_14%_22%)] transition-all duration-100"
                  onClick={() => setLocation(`/hospitals/${h.id}`)}>

                  {/* Hospital name */}
                  <div className="px-4 py-3.5 flex items-center gap-3 min-w-0">
                    <div className="w-7 h-7 shrink-0 border border-[hsl(220_14%_26%)] bg-[hsl(220_14%_18%)] flex items-center justify-center">
        ...
        {!loading && filtered.length > 0 && (
          <div className="border-t border-[hsl(220_14%_18%)] bg-[hsl(220_14%_14%)] px-4 py-2 flex items-center justify-between">
