...
export default function Hospitals() {
  ...
  return (
    <Layout>
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="relative flex-1 max-w-72">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full pl-9 pr-3 py-2 bg-[hsl(220_12%_10%)] border border-[hsl(220_12%_14%)] text-[11px] text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:border-primary/40 focus:ring-1 focus:ring-primary/20 transition tracking-wide" />
        </div>
        <button onClick={() => setShowSuspended(s => !s)}
          className={`flex items-center gap-1.5 px-3 py-2 border text-[10px] font-bold uppercase tracking-[0.15em] transition ${
            showSuspended
              ...
              : "border-[hsl(220_12%_14%)] bg-transparent text-muted-foreground/50 hover:text-muted-foreground hover:border-[hsl(220_12%_20%)]"
          }`}>
      ...
      {/* Table container */}
      <div className="border border-[hsl(220_12%_13%)] overflow-hidden">

        {/* Table header */}
        <div className="border-b border-[hsl(220_12%_13%)] bg-[hsl(220_14%_6%)] grid grid-cols-[1fr_140px_120px_130px_110px_32px] gap-0">
        ...
        {loading ? (
          <div className="flex items-center justify-center gap-2.5 py-16 bg-[hsl(220_14%_7%)]">
            <Loader2 className="w-4 h-4 animate-spin text-muted-foreground/40" />
            <span className="text-[11px] text-muted-foreground/40 uppercase tracking-[0.2em]">Loading registry…</span>
          </div>
        ) : error ? (
          <div className="flex items-center justify-center gap-2 py-16 bg-[hsl(220_14%_7%)]">
            <AlertCircle className="w-4 h-4 text-destructive/70" />
            <span className="text-[11px] text-destructive/70">{error}</span>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 bg-[hsl(220_14%_7%)]">
          ...
          <div className="divide-y divide-[hsl(220_12%_11%)]">
            {filtered.map(h => {
              ...
              return (
                <div key={h.id}
                  className="grid grid-cols-[1fr_140px_120px_130px_110px_32px] items-center cursor-pointer group hover:bg-[hsl(220_14%_9%)] transition-all duration-100"
                  onClick={() => setLocation(`/hospitals/${h.id}`)}>

                  {/* Hospital name */}
                  <div className="px-4 py-3.5 flex items-center gap-3 min-w-0">
                    <div className="w-7 h-7 shrink-0 border border-[hsl(220_12%_16%)] bg-[hsl(220_14%_7%)] flex items-center justify-center">
                  ...
                  <div className="px-4 py-3.5">
                    {expDate ? (
                      <div>
                        <p className={`text-[11px] font-semibold tabular-nums ${isExpired ? "text-red-400" : isExpiringSoon ? "text-[hsl(43_65%_58%)]" : "text-muted-foreground/60"}`}>
                        ...
                        {isExpiringSoon && <p className="text-[9px] text-[hsl(43_65%_58%/0.7)] uppercase tracking-wider mt-0.5">{daysLeft}d left</p>}
                      </div>
                    ) : (
        ...
        {!loading && filtered.length > 0 && (
          <div className="border-t border-[hsl(220_12%_11%)] bg-[hsl(220_14%_6%)] px-4 py-2 flex items-center justify-between">
